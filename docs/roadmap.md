...
| 2.6 | Jira | HMAC-SHA256 `X-Hub-Signature` (Jira admin webhook secret) | `jira.issue.created`, `jira.issue.resolved`, `jira.comment.created` | v0.20.0 (`connectors-jira@0.2.0`) |

### Phase 5 — Jira + database source connectors (v0.18.0–v0.22.0, preview)

- `@statewavedev/connectors-jira` (`0.3.0`) — Jira Cloud REST v3, API-token auth, pull-mode. Issues + opt-in comments → `project:<KEY>`. No-email user fields (displayName/accountId), ADF→plain-text, redaction, project allowlist. `jira.issue.created`, `jira.issue.resolved`, `jira.comment.created`. `0.2.0` adds a webhook receiver (`listen jira`) with verified `X-Hub-Signature` HMAC-SHA256 (see Tier 2 row 2.6). `0.3.0` adds opt-in status transitions (`--include transitions` → `jira.issue.transition`, also emitted by the webhook) and opt-in sprint context (`--sprint-field`).
...
- `@statewavedev/connectors-jira` `0.4.0` adds Jira **Server / Data Center** support (`--deployment server`): REST v2, plain-text bodies, PAT-Bearer/Basic auth. **Implemented + unit-tested, but not yet validated against a live on-prem instance** — tracked in [statewave#193](https://github.com/smaramwbc/statewave/issues/193).
- `@statewavedev/connectors-cli` → `0.2.5` — wires `sync jira` (incl. `--include transitions`, `--sprint-field`, `--deployment server` + `--personal-access-token`) + `sync database` (incl. `--mode schema` + `--tables`) and `listen jira`.
